...
class TargetMemory:
    ...
    def __init__(
        ...
        strong_confirm_count=8,
        strong_confirm_confidence=0.82,
        strong_confirm_age_s=1.0,
        strong_confirm_position_spread_m=2.4,
        strong_confirm_depth_spread_m=3.0,
        ...
        self.strong_confirm_count = strong_confirm_count
        self.strong_confirm_confidence = strong_confirm_confidence
        self.strong_confirm_age_s = strong_confirm_age_s
        self.strong_confirm_position_spread_m = strong_confirm_position_spread_m
        self.strong_confirm_depth_spread_m = strong_confirm_depth_spread_m
    ...
    def _depth_spread(self, candidate):
        depths = candidate["depths"]
        ...
        avg_depth = sum(depths) / len(depths)
        return max(abs(d - avg_depth) for d in depths)

    def _position_spread(self, candidate):
        ns = candidate.get("target_n_list", [])
        es = candidate.get("target_e_list", [])

        if len(ns) < 2 or len(es) < 2:
            return 0.0

        avg_n, avg_e = self._avg_xy(ns, es)
        return max(math.hypot(n - avg_n, e - avg_e) for n, e in zip(ns, es))
    ...
    def _strong_candidate_ready(self, candidate):
        age_s = candidate["last_seen"] - candidate["first_seen"]

        if candidate["count"] < self.strong_confirm_count:
            return False

        if candidate.get("confidence", 0.0) < self.strong_confirm_confidence:
            return False

        if age_s < self.strong_confirm_age_s:
            return False

        if self._position_spread(candidate) > self.strong_confirm_position_spread_m:
            return False

        if self._depth_spread(candidate) > self.strong_confirm_depth_spread_m:
            return False
        ...
        for candidate in self.candidates:
            if self._same_target(candidate, det):
                ...
                if det.get("confidence", 0.0) >= candidate.get("confidence", 0.0):
                    ...
                    candidate["label"] = det.get("label", candidate.get("label"))
                    candidate["source"] = det.get("source", candidate.get("source"))
                ...
                if self._candidate_ready(candidate):
                    ...
                    confirmed["confirmation_reason"] = "normal"
                    self.confirmed.append(confirmed)
                    self.candidates.remove(candidate)
                    return confirmed

                if self._strong_candidate_ready(candidate):
                    confirmed = self._make_confirmed(candidate)
                    confirmed["confirmation_reason"] = "strong_persistent"
                    self.confirmed.append(confirmed)
                    self.candidates.remove(candidate)
                    return confirmed
