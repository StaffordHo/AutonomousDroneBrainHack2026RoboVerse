...
class ObstacleMonitor:
    ...
    def get_depth_frame(self, clean=True):
        """
        Return a copy of the latest depth frame for higher-level planners.

        clean=True applies the same invalid/far/no-hit handling used by the
        clearance monitor, so consumers do not accidentally treat NaN/inf/zero
        pixels as near obstacles.
        """
        if self.latest_depth is None:
            return None

        if clean:
            return self._clean_depth()

        return self.latest_depth.astype(np.float32, copy=True)
