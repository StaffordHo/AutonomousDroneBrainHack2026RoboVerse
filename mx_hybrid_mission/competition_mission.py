...
from local_rrt_planner import LocalRRTPlanner, heading_between_deg
...
MISSION_TIME_LIMIT_S = float(os.getenv("MISSION_TIME_LIMIT_S", str(9 * 60)))
LANDING_BUFFER_S = float(os.getenv("LANDING_BUFFER_S", "18"))
...
OPEN_CRUISE_STEP_M = float(os.getenv("OPEN_CRUISE_STEP_M", "0.58"))
OPEN_CRUISE_MIN_FRONT_M = float(os.getenv("OPEN_CRUISE_MIN_FRONT_M", "8.0"))
OPEN_CRUISE_MIN_SIDE_M = float(os.getenv("OPEN_CRUISE_MIN_SIDE_M", "3.0"))
OPEN_CRUISE_MIN_LOWER_M = float(os.getenv("OPEN_CRUISE_MIN_LOWER_M", "2.0"))
OPEN_CRUISE_MAX_TURN_DEG = float(os.getenv("OPEN_CRUISE_MAX_TURN_DEG", "35.0"))
...
UNCHARTED_FRONTIER_ENABLED = os.getenv("UNCHARTED_FRONTIER_ENABLED", "1") == "1"
UNCHARTED_NEW_CELL_BONUS = float(os.getenv("UNCHARTED_NEW_CELL_BONUS", "4.5"))
UNCHARTED_REVISIT_PENALTY = float(os.getenv("UNCHARTED_REVISIT_PENALTY", "5.0"))
UNCHARTED_NARROW_BONUS = float(os.getenv("UNCHARTED_NARROW_BONUS", "22.0"))
UNCHARTED_NEAR_HOME_RADIUS_M = float(os.getenv("UNCHARTED_NEAR_HOME_RADIUS_M", "7.0"))
UNCHARTED_NEAR_HOME_OUTWARD_BONUS = float(
    os.getenv("UNCHARTED_NEAR_HOME_OUTWARD_BONUS", "8.0")
)
NBV_FRONTIER_ENABLED = os.getenv("NBV_FRONTIER_ENABLED", "1") == "1"
NBV_FRONTIER_RINGS_M = [
    float(value)
    for value in os.getenv("NBV_FRONTIER_RINGS_M", "6,10,14,18").split(",")
    if value.strip()
]
NBV_FRONTIER_SECTORS = int(os.getenv("NBV_FRONTIER_SECTORS", "16"))
NBV_FRONTIER_GOALS_PER_PASS = int(os.getenv("NBV_FRONTIER_GOALS_PER_PASS", "6"))
NBV_FRONTIER_VIEW_RADIUS_M = float(os.getenv("NBV_FRONTIER_VIEW_RADIUS_M", "3.2"))
NBV_FRONTIER_MAX_STEPS = int(os.getenv("NBV_FRONTIER_MAX_STEPS", "16"))
NBV_FRONTIER_SCAN_FRAMES = int(os.getenv("NBV_FRONTIER_SCAN_FRAMES", "2"))
NBV_FRONTIER_ATTEMPT_PENALTY = float(os.getenv("NBV_FRONTIER_ATTEMPT_PENALTY", "18.0"))
NBV_FRONTIER_TRAVEL_PENALTY = float(os.getenv("NBV_FRONTIER_TRAVEL_PENALTY", "0.45"))
NBV_FRONTIER_RING_BONUS = float(os.getenv("NBV_FRONTIER_RING_BONUS", "1.4"))
NBV_FRONTIER_MIN_INFO_GAIN = float(os.getenv("NBV_FRONTIER_MIN_INFO_GAIN", "8.0"))
...
# Obstacle / safety
MAX_ATTITUDE_DEG = float(os.getenv("MAX_ATTITUDE_DEG", "8.0"))
MID_STEP_MAX_ATTITUDE_DEG = float(os.getenv("MID_STEP_MAX_ATTITUDE_DEG", "12.0"))
CRITICAL_ATTITUDE_DEG = float(os.getenv("CRITICAL_ATTITUDE_DEG", "25.0"))
RECOVERY_ATTITUDE_TOLERANCE_DEG = float(os.getenv("RECOVERY_ATTITUDE_TOLERANCE_DEG", "15.0"))
RECOVERY_HARD_LIMIT_DEG = float(os.getenv("RECOVERY_HARD_LIMIT_DEG", "35.0"))
MIN_FRONT_MOVE_CLEARANCE_M = float(os.getenv("MIN_FRONT_MOVE_CLEARANCE_M", "1.00"))
MIN_SIDE_MOVE_CLEARANCE_M = float(os.getenv("MIN_SIDE_MOVE_CLEARANCE_M", "0.45"))
MIN_LOWER_MOVE_CLEARANCE_M = float(os.getenv("MIN_LOWER_MOVE_CLEARANCE_M", "0.45"))
MID_STEP_ABORT_CLEARANCE_M = float(os.getenv("MID_STEP_ABORT_CLEARANCE_M", "0.90"))
YAW_MIN_FRONT_CLEARANCE_M = float(os.getenv("YAW_MIN_FRONT_CLEARANCE_M", "0.90"))
YAW_MIN_SIDE_CLEARANCE_M = float(os.getenv("YAW_MIN_SIDE_CLEARANCE_M", "0.40"))
NARROW_CORRIDOR_ENABLED = os.getenv("NARROW_CORRIDOR_ENABLED", "1") == "1"
NARROW_CORRIDOR_MIN_FRONT_M = float(os.getenv("NARROW_CORRIDOR_MIN_FRONT_M", "1.05"))
NARROW_CORRIDOR_MIN_SIDE_M = float(os.getenv("NARROW_CORRIDOR_MIN_SIDE_M", "0.40"))
NARROW_CORRIDOR_STEP_M = float(os.getenv("NARROW_CORRIDOR_STEP_M", "0.20"))
NARROW_UNCHARTED_STEP_M = float(os.getenv("NARROW_UNCHARTED_STEP_M", "0.18"))
NARROW_UNCHARTED_FRONT_M = float(os.getenv("NARROW_UNCHARTED_FRONT_M", "1.65"))
NARROW_UNCHARTED_SIDE_M = float(os.getenv("NARROW_UNCHARTED_SIDE_M", "0.80"))
...
ALT_TOLERANCE_M = float(os.getenv("ALT_TOLERANCE_M", "1.0"))
CRITICAL_ALTITUDE_DEVIATION_M = float(os.getenv("CRITICAL_ALTITUDE_DEVIATION_M", "3.5"))
...
MAX_FAILED_STEPS_PER_GOAL = int(os.getenv("MAX_FAILED_STEPS_PER_GOAL", "3"))
MAX_FAILED_HEADINGS_PER_PASS = 4

# Experimental local RRT assist. The frontier planner still chooses the mission
# direction; RRT is only asked for a short escape/revisit route after repeated
# failed moves.
RRT_ASSIST_ENABLED = os.getenv("RRT_ASSIST_ENABLED", "1") == "1"
RRT_ASSIST_MIN_FAILED_STEPS = int(os.getenv("RRT_ASSIST_MIN_FAILED_STEPS", "2"))
RRT_ASSIST_MAX_RANGE_M = float(os.getenv("RRT_ASSIST_MAX_RANGE_M", "7.5"))
RRT_ASSIST_GOAL_DISTANCE_M = float(os.getenv("RRT_ASSIST_GOAL_DISTANCE_M", "6.0"))
RRT_ASSIST_STEP_M = float(os.getenv("RRT_ASSIST_STEP_M", "0.8"))
RRT_ASSIST_MAX_ITERATIONS = int(os.getenv("RRT_ASSIST_MAX_ITERATIONS", "160"))
RRT_ASSIST_GOAL_SAMPLE_RATE = float(os.getenv("RRT_ASSIST_GOAL_SAMPLE_RATE", "0.30"))
RRT_ASSIST_OBSTACLE_RADIUS_M = float(os.getenv("RRT_ASSIST_OBSTACLE_RADIUS_M", "0.85"))
...
rrt_planner = LocalRRTPlanner(
    max_range_m=RRT_ASSIST_MAX_RANGE_M,
    step_m=RRT_ASSIST_STEP_M,
    max_iterations=RRT_ASSIST_MAX_ITERATIONS,
    goal_sample_rate=RRT_ASSIST_GOAL_SAMPLE_RATE,
    obstacle_radius_m=RRT_ASSIST_OBSTACLE_RADIUS_M,
)
...
async def move_in_heading(
    ...
    elif front <= NARROW_UNCHARTED_FRONT_M or min(c["left"], c["right"]) <= NARROW_UNCHARTED_SIDE_M:
        step_m = NARROW_UNCHARTED_STEP_M
    elif (
        front >= OPEN_CRUISE_MIN_FRONT_M
        and min(c["left"], c["right"]) >= OPEN_CRUISE_MIN_SIDE_M
        and c["lower_center"] >= OPEN_CRUISE_MIN_LOWER_M
        and angle_diff_deg(command_heading, latest_attitude["yaw"]) <= OPEN_CRUISE_MAX_TURN_DEG
    ):
        step_m = OPEN_CRUISE_STEP_M
    elif (
...
def rrt_blocked_points():
    if exploration_memory.start_n is None or exploration_memory.start_e is None:
        return []

    return [
        (
            exploration_memory.start_n + cell_n * exploration_memory.cell_size_m,
            exploration_memory.start_e + cell_e * exploration_memory.cell_size_m,
        )
        for cell_n, cell_e in exploration_memory.blocked_cells
    ]


def rrt_live_clearance_for_heading(heading_deg):
    clearances = monitor.get_directional_clearance()
    relative_heading = normalize_angle_deg(heading_deg - latest_attitude["yaw"])
    return clearance_for_relative_heading(clearances, relative_heading)


def rrt_point_score(point_n, point_e):
    if latest_position_ned is None:
        return 0.0

    cell = exploration_memory._cell(point_n, point_e)
    visit_count = exploration_memory.visited_cells.get(cell, 0)
    score = 5.0 if visit_count == 0 else -1.5 * visit_count

    current_range = distance_from_start()
    point_range = math.hypot(point_n - start_n, point_e - start_e)

    if current_range < SOFT_RANGE_LIMIT_M - 1.0:
        score += clamp(point_range - current_range, -1.0, 2.0)
    else:
        score -= max(0.0, point_range - current_range) * 2.5

    return score


def local_information_gain(center_n, center_e, radius_m=NBV_FRONTIER_VIEW_RADIUS_M):
    if exploration_memory.start_n is None or exploration_memory.start_e is None:
        return 0.0, {"new_cells": 0, "visited_cells": 0, "blocked_cells": 0}

    cell_size = exploration_memory.cell_size_m
    radius_cells = max(1, int(math.ceil(radius_m / cell_size)))
    center_cell_n, center_cell_e = exploration_memory._cell(center_n, center_e)
    ...
    for dn in range(-radius_cells, radius_cells + 1):
        for de in range(-radius_cells, radius_cells + 1):
            distance_cells = math.hypot(dn, de)
            if distance_cells > radius_cells:
                continue

            cell = (center_cell_n + dn, center_cell_e + de)
            weight = 1.0 - 0.55 * (distance_cells / max(radius_cells, 1))
            ...
            if visit_count == 0:
                ...
                visited_cells += 1
                score -= min(visit_count, 4) * 0.85 * weight
...
def nbv_frontier_viewpoints(max_goals=NBV_FRONTIER_GOALS_PER_PASS):
    if latest_position_ned is None:
        return []
    ...
    for ring_index, ring_m in enumerate(NBV_FRONTIER_RINGS_M):
        if ring_m > SOFT_RANGE_LIMIT_M - 0.6:
            continue

        for sector_index in range(NBV_FRONTIER_SECTORS):
            sector_width_deg = 360.0 / max(1, NBV_FRONTIER_SECTORS)
            heading = normalize_angle_deg(
                start_yaw + sector_index * sector_width_deg + 0.5 * sector_width_deg
            )
            heading_rad = math.radians(heading)
            goal_n = start_n + ring_m * math.cos(heading_rad)
            goal_e = start_e + ring_m * math.sin(heading_rad)
            cell = exploration_memory._cell(goal_n, goal_e)

            if cell in exploration_memory.blocked_cells:
                continue

            direct_heading = normalize_angle_deg(
                math.degrees(math.atan2(goal_e - current_e, goal_n - current_n))
            )
            relative_heading = normalize_angle_deg(direct_heading - current_yaw)
            direct_clearance = clearance_for_relative_heading(clearances, relative_heading)
            info_gain, debug = local_information_gain(goal_n, goal_e)
            visit_count = exploration_memory.visited_cells.get(cell, 0)
            attempts = nbv_frontier_attempts.get((ring_index, sector_index), 0)
            distance_to_goal = math.hypot(goal_n - current_n, goal_e - current_e)

            if info_gain < NBV_FRONTIER_MIN_INFO_GAIN and attempts > 0:
                continue

            score = info_gain
            score += ring_m * NBV_FRONTIER_RING_BONUS
            score += min(direct_clearance, 8.0) * 1.2
            score -= distance_to_goal * NBV_FRONTIER_TRAVEL_PENALTY
            score -= visit_count * UNCHARTED_REVISIT_PENALTY
            score -= attempts * NBV_FRONTIER_ATTEMPT_PENALTY

            if debug["new_cells"] >= debug["visited_cells"]:
                score += UNCHARTED_NARROW_BONUS * 0.35
            ...
            candidates.append(
                {
                    ...
                    "score": score,
                    "info_gain": info_gain,
                    "new_cells": debug["new_cells"],
                    "visited_cells": debug["visited_cells"],
                    "blocked_cells": debug["blocked_cells"],
                    "attempts": attempts,
    ...
    for candidate in candidates:
        ...
        too_close = any(
            math.hypot(candidate["goal_n"] - existing["goal_n"], candidate["goal_e"] - existing["goal_e"])
            < NBV_FRONTIER_VIEW_RADIUS_M
...
async def nbv_frontier_explore_pass(drone, target_down, pass_name, max_goals=NBV_FRONTIER_GOALS_PER_PASS):
    global active_allowed_colours
    global active_target_alt_d

    if not NBV_FRONTIER_ENABLED:
        return 0

    active_allowed_colours = ("yellow", "red")
    active_target_alt_d = target_down
    goals = nbv_frontier_viewpoints(max_goals=max_goals)

    if not goals:
        print(f"\n===== {pass_name} NBV skipped: no useful frontier viewpoints =====")
        return 0

    print(f"\n===== {pass_name} NBV_FRONTIER goals={len(goals)} alt_d={target_down:.1f} =====")
    completed = 0
    ...
    for index, goal in enumerate(goals):
        ...
        key = (goal["ring_index"], goal["sector_index"])
        nbv_frontier_attempts[key] = nbv_frontier_attempts.get(key, 0) + 1

        print(
            f"🧭 {pass_name}: NBV {index:02d} ring={goal['ring_m']:.1f} "
            f"sector={goal['sector_index']:02d} score={goal['score']:.1f} "
            f"new={goal['new_cells']} visited={goal['visited_cells']} "
            f"dist={goal['distance']:.1f} clr={goal['direct_clearance']:.1f}"
        )

        reached = await navigate_to_coverage_goal(
            drone,
            goal["goal_n"],
            goal["goal_e"],
            target_down,
            label=f"{pass_name}_NBV_{index:02d}",
            max_steps=NBV_FRONTIER_MAX_STEPS,
        )
        ...
        if NBV_FRONTIER_SCAN_FRAMES > 0:
            await scan_current_view(
                label=f"{pass_name}_NBV_{index:02d}_view",
                frames=NBV_FRONTIER_SCAN_FRAMES,
            )

    return completed


def rrt_plan_heading_to_goal(goal_n, goal_e, label, preferred_heading=None):
    if not RRT_ASSIST_ENABLED or latest_position_ned is None:
        return None

    if not vehicle_stable(active_target_alt_d):
        print(
            f"🧪 [{label}] RRT assist skipped: vehicle unstable "
            f"pitch={latest_attitude['pitch']:.1f} roll={latest_attitude['roll']:.1f}"
        )
        return None

    start = (latest_position_ned.north_m, latest_position_ned.east_m)
    path = rrt_planner.plan(
        start=start,
        goal=(goal_n, goal_e),
        blocked_points=rrt_blocked_points(),
        clearance_provider=rrt_live_clearance_for_heading,
        score_point=rrt_point_score,
    )
    ...
    waypoint = path[1]
    heading = heading_between_deg(start, waypoint)

    if preferred_heading is not None and angle_diff_deg(heading, preferred_heading) > 130.0:
        return None

    print(
        f"🧪 [{label}] RRT assist path={len(path)} "
        f"first_heading={heading:.1f} goal_dist={distance_to_point_m(goal_n, goal_e):.1f}m"
    )
    log_mission_action(
        action_type="RRT_ASSIST",
        label=label,
        target_down=active_target_alt_d,
        preferred_heading=preferred_heading,
        selected_heading=heading,
        heading_score=len(path),
        extra={
            "goal_n": goal_n,
            "goal_e": goal_e,
            "path_points": len(path),
            "blocked_points": len(rrt_blocked_points()),
        },
    )
    return heading


def rrt_escape_goal(preferred_heading=None):
    if latest_position_ned is None:
        return None
    ...
    if preferred_heading is not None:
        headings.extend(
            normalize_angle_deg(preferred_heading + offset)
            for offset in [0, 20, -20, 45, -45, 80, -80]
        )

    headings.extend(
        normalize_angle_deg(current_yaw + offset)
        for offset in [0, 25, -25, 55, -55, 90, -90, 135, -135, 180]
    )
    ...
    for heading in headings:
        ...
        if direction_clearance < FRONTIER_MIN_DIRECTION_CLEARANCE_M:
            continue

        heading_rad = math.radians(heading)
        goal_distance = min(RRT_ASSIST_GOAL_DISTANCE_M, max(2.5, direction_clearance - 0.6))
        goal_n = current_n + goal_distance * math.cos(heading_rad)
        goal_e = current_e + goal_distance * math.sin(heading_rad)
        projected_score, debug = exploration_memory.projected_path_score(
            current_n=current_n,
            current_e=current_e,
        ...
        score = projected_score + min(direction_clearance, 10.0) * 1.5
        score += debug["new_cells"] * 2.0
        score -= angle_diff_deg(heading, current_yaw) * 0.025

        if preferred_heading is not None:
            score -= angle_diff_deg(heading, preferred_heading) * 0.04

        if score > best_score:
            best_score = score
            best_goal = (goal_n, goal_e, heading)

    return best_goal


async def try_rrt_escape_move(drone, target_down, label, preferred_heading=None):
    if latest_position_ned is None:
        return False
    ...
    if not vehicle_stable(target_down):
        print(
            f"🧪 [{label}_rrt] RRT assist skipped: vehicle unstable "
            f"pitch={latest_attitude['pitch']:.1f} roll={latest_attitude['roll']:.1f}"
        )
        return False

    goal = rrt_escape_goal(preferred_heading=preferred_heading)
    if goal is None:
        return False

    goal_n, goal_e, goal_heading = goal
    rrt_heading = rrt_plan_heading_to_goal(
        goal_n,
        goal_e,
        label=f"{label}_rrt",
        preferred_heading=preferred_heading or goal_heading,
    )
    if rrt_heading is None:
        return False
...
def choose_frontier_heading(preferred_heading_deg=None):
    ...
    for heading in candidates:
        ...
        if UNCHARTED_FRONTIER_ENABLED and deep_mode:
            score += projected_debug["new_cells"] * UNCHARTED_NEW_CELL_BONUS
            score -= projected_debug["revisits"] * UNCHARTED_REVISIT_PENALTY

            if projected_debug["new_cells"] >= 2 and direction_clearance <= 3.0:
                score += UNCHARTED_NARROW_BONUS
        ...
        if corridor_heading and side_clearance < MIN_SIDE_MOVE_CLEARANCE_M:
            ...
            if UNCHARTED_FRONTIER_ENABLED and deep_mode:
                if projected_debug["new_cells"] > projected_debug["revisits"]:
                    score += UNCHARTED_NARROW_BONUS * 0.75
                else:
                    score -= UNCHARTED_REVISIT_PENALTY * 2.0
        ...
        if UNCHARTED_FRONTIER_ENABLED and deep_mode and current_range < UNCHARTED_NEAR_HOME_RADIUS_M:
            score += max(0.0, range_gain) * UNCHARTED_NEAR_HOME_OUTWARD_BONUS
            score -= max(0.0, -range_gain) * UNCHARTED_NEAR_HOME_OUTWARD_BONUS * 1.5
...
async def frontier_stride(
    ...
    for step in range(steps_per_stride):
        ...
        if moved:
            ...
            if failed_steps >= RRT_ASSIST_MIN_FAILED_STEPS:
                rrt_moved = await try_rrt_escape_move(
                    drone,
                    target_down,
                    label=f"{label}_{step:02d}",
                    preferred_heading=preferred_heading or heading,
                )

                if rrt_moved:
                    moved_steps += 1
                    failed_steps = 0
                    preferred_heading = latest_attitude["yaw"]
                    await handle_candidate_event(drone, target_down)
                    continue
        ...
        if moved:
            ...
            if failed_steps >= RRT_ASSIST_MIN_FAILED_STEPS:
                rrt_heading = rrt_plan_heading_to_goal(
                    goal_n,
                    goal_e,
                    label=f"{label}_{step:02d}_rrt",
                    preferred_heading=heading,
                )

                if rrt_heading is not None:
                    rrt_moved = await move_in_heading(
                        drone,
                        rrt_heading,
                        target_down,
                        label=f"{label}_{step:02d}_rrt",
...
async def main():
    ...
    print(
        "RRT assist: "
        f"enabled={RRT_ASSIST_ENABLED} range={RRT_ASSIST_MAX_RANGE_M:.1f}m "
        f"iters={RRT_ASSIST_MAX_ITERATIONS} trigger_failures={RRT_ASSIST_MIN_FAILED_STEPS}"
    )
    print(
        "Uncharted frontier bias: "
        f"enabled={UNCHARTED_FRONTIER_ENABLED} narrow_bonus={UNCHARTED_NARROW_BONUS:.1f} "
        f"near_home_radius={UNCHARTED_NEAR_HOME_RADIUS_M:.1f}m "
        f"narrow_step={NARROW_UNCHARTED_STEP_M:.2f}m"
    )
    print(
        "NBV frontier layer: "
        f"enabled={NBV_FRONTIER_ENABLED} rings={NBV_FRONTIER_RINGS_M} "
        f"sectors={NBV_FRONTIER_SECTORS} goals/pass={NBV_FRONTIER_GOALS_PER_PASS}"
    )
    print(
        "Deep coverage profile: "
        f"soft={SOFT_RANGE_LIMIT_M:.1f}m resume={RESUME_RANGE_M:.1f}m "
        f"hard={HARD_RANGE_LIMIT_M:.1f}m cruise_step={OPEN_CRUISE_STEP_M:.2f}m "
        f"fast_step={FAST_OPEN_STEP_M:.2f}m "
            ...
            if eligibility_met(summary) and not EXTRA_SCORING_AFTER_ELIGIBILITY:
                if CONTINUE_FRONTIER_AFTER_ELIGIBILITY:
                    ...
                    while search_time_remaining():
                        ...
                        nbv_completed = await nbv_frontier_explore_pass(
                            drone,
                            target_down=target_down,
                            pass_name=f"NBV_WORLD_EXPLORE_{cycle:02d}",
                            max_goals=NBV_FRONTIER_GOALS_PER_PASS,
                        )
