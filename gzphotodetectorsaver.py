...
class GZPhotoDetectorSaver:
    def __init__(
        ...
        if not enable_yolo:
            print("YOLO burst detection disabled. Fast HSV mission detector remains active.")
            self.model = None
            ...
            if YOLO is None:
                ...
                print(
                    ...
                    f"(Threshold: {self.threshold}, imgsz={self.imgsz}, device={self.device})"
                )
        ...
        if enable_yolo and self.model is not None:
            print(
                "YOLO burst detection is opt-in. Lower CPU with "
                "YOLO_BURST_ENABLED=0, YOLO_IMGSZ=320, or YOLO_DEVICE=cpu."
            )
